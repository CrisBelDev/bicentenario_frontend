...
function ListarEventoCultural() {
	...
	useEffect(() => {
		...
		const consultarApi = async () => {
			setCargando(true);
			try {
				const { data } = await usuariosAxios.get("/evento-cultural"); // Cambié la ruta aquí
	...
	const handleBusqueda = (e) => {
		...
		const filtro = eventos.filter((evento) =>
			`${evento.titulo} ${evento.descripcion} ${evento.tipo_evento}`
				.toLowerCase()
	...
	const columnas = [
		...
		{
			name: "Fecha Fin",
			selector: (row) => formatearFecha(row.fecha_finalizacion),
		},
		{
			name: "Tipo",
			selector: (row) => row.tipo_evento,
		},
		{
			name: "Acciones",
			cell: (row) => (
				<div className="d-flex gap-1">
					<Link
						to={`/bicentenario-dashboard/detalle-evento-cultural/${row.id_evento_cultural}`}
					...
					<button
						...
						onClick={() => handleEliminarEvento(row.id_evento_cultural)}
					>
...
export default ListarEventoCultural;
